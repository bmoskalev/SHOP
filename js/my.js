...
function renderOrder() {
    ...
    $.ajax({
        ...
        success: function (dateAnswer) {
            ...
            for (let key in dateAnswerBasket) {
            ...
            for (let key in dateAnswerBasket) {
                ...
                let goodDiscount;
                if (dateAnswerBasket[key].discount > 0) {
                    goodDiscount = dateAnswerBasket[key].count * dateAnswerBasket[key].price * ((100 - dateAnswerBasket[key].discount) / 100);
                } else {
                    goodDiscount = dateAnswerBasket[key].count * dateAnswerBasket[key].price;
                }
...
function renderOrderEnd() {
    ...
    $.ajax({
        ...
        success: function (dateAnswer) {
            ...
            let date = new Date(dateAnswer[0].timeOrder * 1000);// Hours part from the timestamp
            let hours = date.getHours();// Minutes part from the timestamp
            let minutes = "0" + date.getMinutes();// Seconds part from the timestamp
            let formattedTime = hours + ':' + minutes.substr(-2);// Will display time in 10:30:23 format
            ...
            let totalCoast = Math.floor(sumGoodDiscount - happyHours - delivery);
            ...
            let orderTableEnd = $('.orderTableEnd');
            orderTableEnd.empty();
